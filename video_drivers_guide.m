...
# ВЕРСИЯ ДРАЙВЕРА: NVIDIA предлагает два основных типа драйверов:
# - Production Branch/Studio: Выбирайте для оптимальной стабильности и производительности. Рекомендуется большинству пользователей.
# - New Feature Branch (NFB)/Quadro New Feature (QNF): Выбирайте, если вам требуются новые функции или поддержка новой ОС между релизами Production Branch. Поддержка короче.
# Убедитесь, что выбранная вами версия драйвера совместима с вашей моделью GPU (проверьте на https://www.nvidia.com/drivers/).


# A. Для стандартного ядра (linux) - Production Branch/Studio:

# pacman -S --noconfirm nvidia nvidia-utils nvidia-settings lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia


# B. Для LTS-ядра (linux-lts) - Production Branch/Studio:

# pacman -S --noconfirm nvidia-lts nvidia-utils nvidia-settings lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia


# C. Универсальный DKMS-драйвер (Production Branch/Studio, если стандартный не подходит или кастомное ядро):

# pacman -S --noconfirm nvidia-dkms nvidia-utils nvidia-settings lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia

# (Если вы хотите использовать New Feature Branch, замените 'nvidia' на 'nvidia-beta', 'nvidia-lts' на 'nvidia-beta-lts', или 'nvidia-dkms' на 'nvidia-beta-dkms' в соответствующем варианте A, B или C.)
...
# ШАГ 4: ДОПОЛНИТЕЛЬНО (только для аппаратного ускорения видео (VA-API) на NVIDIA):
...
# Установка на систему с гибридной графикой (например, Intel + NVIDIA) может КОНФЛИКТОВАТЬ с VA-API через Mesa (интегрированную графику).
