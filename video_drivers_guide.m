...
# pacman -S --noconfirm vulkan-intel intel-media-driver mesa
...
### ПРИМЕЧАНИЕ: Совместимость с Wayland может быть ограничена. Проверьте совместимость для вашего DE/WM.
##############################################################
...
# ШАГ 1.1 (дополнительно): VA-API для NVIDIA.
# Пакет libva-nvidia-driver обеспечивает VA-API через проприетарный интерфейс NVIDIA.
# УСТАНОВКА libva-nvidia-driver НЕ РЕКОМЕНДУЕТСЯ по умолчанию и может КОНФЛИКТОВАТЬ с VA-API через Mesa (интегрированная графика).
# УСТАНАВЛИВАЙТЕ libva-nvidia-driver ТОЛЬКО ЕСЛИ У ВАС В СИСТЕМЕ УСТАНОВЛЕНА ИСКЛЮЧИТЕЛЬНО ВИДЕОКАРТА NVIDIA (без интегрированной Intel/AMD).
# В гибридных системах (Intel/AMD + NVIDIA) для VA-API обычно используется Mesa (установлен в Шаге 1 Варианта 1 или 2).
# Если вы НЕ УВЕРЕНЫ, что он нужен, ПРОПУСТИТЕ его установку.
# Если вы решили его установить, выполните команду (после установки основных драйверов):
...
# Это современный способ запуска приложений на дискретной GPU в Wayland.
# Работает как на ноутбуках, так и на некоторых настольных системах с поддержкой DCC.
# ПРИМЕЧАНИЕ: switcherooctl требует поддержки DCC на уровне оборудования и может не работать на всех системах.
...
# echo 'options nvidia NVreg_DynamicPowerManagement=0x02' >> /etc/modprobe.d/nvidia.conf # Для ноутбуков (опционально, может увеличить энергопотребление)
# # echo 'nvidia-drm' > /etc/modules-load.d/nvidia-drm.conf # Обычно не требуется, nvidia-drm загружается с nvidia

# Настройка переменных окружения для совместимости (например, для legacy приложений) в /etc/environment:
# ПРИМЕЧАНИЕ: Эти переменные (ниже) могут быть устаревшими для switcherooctl или новых версий Mesa/Vulkan.
# Их установка НЕ влияет на запуск приложений через switcherooctl в Wayland.

# echo -e '__NV_PRIME_RENDER_OFFLOAD=1\n__VK_LAYER_NV_optimus=NVIDIA_only' >> /etc/environment # Эти переменные могут быть устаревшими для switcherooctl

##############################################################

# 3. НАСТРОЙКА INITRAMFS (mkinitcpio)
# -----------------------------------
# А. Модули: Обычно не требуется вручно добавлять модули NVIDIA.
...
# Примечание: При использовании Plymouth с NVIDIA может потребоваться добавить `nvidia` в `MODULES`.

# Б. Хуки: Убедитесь, что в HOOKS присутствует kms (удаление вредно!).
# Порядок для гибернации: (base udev ...) [plymouth] (autodetect modconf) kms (...) [resume] (...) [filesystems] fsck
...
# Поддержка VA-API для nouveau поставляется с пакетом 'mesa', установленным в Шаге 1 Варианта 1 или 2.
# pacman -S --noconfirm libva-mesa-driver # Этот пакет не существует, VA-API для nouveau входит в 'mesa'.
...
# 4. ПРОЧИТАЙТЕ РАЗДЕЛ "ОПЦИОНАЛЬНЫЕ НАСТРОЙКИ ДЛЯ НОУТБУКОВ" (см. выше) и выполните его *в chroot*.
...
# 6. После завершения установки DE/WM *внутри chroot*, ВЕРНИТЕСЬ К ЭТОМУ файлу.
...
# Утилита 'vainfo' поставляется с пакетом 'libva-utils'. Убедитесь, что он установлен.
# pacman -S libva-utils # Выполните, если vainfo не найден
# VA-API будет работать с драйвером, предоставленным пакетом 'mesa' (Intel/AMD/nouveau) или, если установлен, 'libva-nvidia-driver' (NVIDIA).
vainfo
...
# Проверка Prime Offloading (устаревшее, для Xorg) (только для NVIDIA с проприетарными драйверами):
# Эта проверка НЕ актуальна для Wayland и switcherooctl.
...
# В. Интеграция с графическими оболочками (Wayland):
...
# 3. Xfce, LXDE и другие: Обычно отсутствует интеграция в Wayland. Используйте терминал или создайте .desktop файл.
...
# ПРИМЕЧАНИЯ:
# - Совместимость проприетарных драйверов NVIDIA с Wayland может быть ограничена. Проверьте совместимость для вашего конкретного DE/WM.
# - В Wayland для запуска приложений на определённой GPU используйте switcherooctl. Устаревшие методы (DRI_PRIME, переменные окружения) не применяются к запуску в Wayland.
# - Рекомендуется сделать резервную копию конфигурационных файлов (например, /etc/X11/xorg.conf, если будет создан) перед внесением изменений.
# - Пакеты lib32-* (например, lib32-opencl-nvidia) требуются для 32-битных приложений (включая игры). Установите их, если планируете использовать такие приложения.
