...
### + Информация о версиях драйверов NVIDIA и VA-API
...
# ВАЖНО О ПОКОЛЕНИЯХ GPU:
# Совместимость драйвера также зависит от поколения вашей видеокарты.
# Примеры поколений NVIDIA: Fermi, Kepler, Maxwell, Pascal, Turing, Ampere, Ada Lovelace.
# Старые поколения могут не поддерживаться новыми драйверами.
# Новые карты (Turing, Ampere, Ada Lovelace) требуют современные драйверы.
# Проверка совместимости на сайте NVIDIA обязательна.
...
# 📌 ШАГ 1: ВЫБЕРИТЕ ДРАЙВЕР И ВЕРСИЮ. Установите ТОЛЬКО ОДИН пакет драйвера (nvidia, nvidia-lts, nvidia-dkms) и соответствующие утилиты.
# Убедитесь, что выбранная вами версия драйвера совместима с вашей моделью GPU (проверьте на https://www.nvidia.com/drivers/    ).


# ПОЯСНЕНИЕ К ВЫБОРУ ВЕТКИ ДРАЙВЕРА:
# Производители драйверов NVIDIA предлагают разные ветки:
# 1. Production Branch (Стабильная ветка):
#    - Рекомендуется для большинства пользователей.
#    - Предоставляет оптимальную стабильность и производительность.
#    - Имеет длительную поддержку и сертификацию ISV (Independent Software Vendor).
#    - Это переименованная версия Quadro Optimal Driver for Enterprise (ODE).
# 2. New Feature Branch (NFB) / Quadro New Feature (QNF):
#    - Выбирайте, если вам требуются новые функции, исправления ошибок
#      или поддержка новой ОС между релизами Production Branch.
#    - Продолжительность поддержки короче, чем у Production Branch.
# ВАЖНО: Убедитесь, что выбранная вами версия драйвера совместима
# с вашей моделью GPU (проверьте на https://www.nvidia.com/drivers/  ).
# Также убедитесь, что она совместима с версией вашего ядра (linux, linux-lts).
...
# echo 'options nvidia-drm modeset=1' > /etc/modprobe.d/nvidia.conf # Включение режима модсета для NVIDIA (требуется для Wayland)
# echo 'options nvidia NVreg_DynamicPowerManagement=0x02' >> /etc/modprobe.d/nvidia.conf # Для ноутбуков (опционально)

# ШАГ 4: ЕСЛИ NVIDIA GPU является ЕДИНСТВЕННОЙ видеокартой:
# ВАЖНО: Установка libva-nvidia-driver НЕ РЕКОМЕНДУЕТСЯ по умолчанию.
# Устанавливайте ТОЛЬКО если NVIDIA GPU является ЕДИНСТВЕННОЙ видеокартой в системе И вам требуется аппаратное ускорение видео (декодирование/кодирование) на ней.
# Установка на систему с гибридной графикой (Intel/AMD + NVIDIA) может КОНФЛИКТОВАТЬ с VA-API через Mesa (интегрированную графику).
# После установки vainfo должен будет отображать NVIDIA GPU как доступное устройство VA-API.

# pacman -S --noconfirm libva-nvidia-driver # <-- РАСКОММЕНТИРУЙТЕ ТОЛЬКО ЕСЛИ НЕОБХОДИМО И УСЛОВИЯ ВЫШЕ ВЫПОЛНЕНЫ



##############################################################

### 🛠️ ИНСТРУМЕНТ ДЛЯ СВЯЗИ МЕЖДУ ПРИЛОЖЕНИЯМИ И АППАРАТНЫМ ОБЕСПЕЧЕНИЕМ:
...
mkinitcpio -P # Пересборка initramfs с учетом новых модулей драйверов
...
# Проверка VA-API:
# ТЕСТ ПРОВОДИТСЯ ИСКЛЮЧИТЕЛЬНО НА КОМПЬЮТЕРАХ, ГДЕ NVIDIA GPU ЯВЛЯЕТСЯ ЕДИНСТВЕННОЙ ВИДЕО КАРТОЙ.
# Если вы НЕ устанавливали libva-nvidia-driver, vainfo может не показывать NVIDIA GPU.
# Если вы УСТАНОВИЛИ libva-nvidia-driver, он должен отображаться как доступное устройство.

# pacman -S libva-utils # <-- Установите если ещё не установлен
...
# ########################################################
# 📋 7. АВТОМАТИЗИРОВАННАЯ ПРОВЕРКА (по желанию)
# ########################################################
#
# Вы можете создать простой скрипт для выполнения всех проверок.
# 1. Откройте терминал в графической оболочке.
# 2. Создайте файл:
    nano ~/check_gpu.sh
# 3. Вставьте следующий текст (для NVIDIA):
    #!/bin/bash
    echo "=== Проверка NVIDIA ==="
    nvidia-smi
    echo -e "\n=== Проверка Vulkan ==="
    vulkaninfo --summary
    echo -e "\n=== Проверка VA-API ==="
    ...
    vainfo
    echo -e "\n=== Проверка switcherooctl (если установлен) ==="
    switcherooctl list
    echo -e "\n=== Визуальный тест (glmark2) ==="
    glmark2
