...
2.  **Открытие макета:**
    *   Откройте файл макета установки (например, `Arch UEFI.sh`) в **текстовом редакторе**.
...
*   `Arch UEFI.sh`: Основной макет установки для систем с UEFI.
*   `Arch BIOS.sh`: Основной макет установки для систем с BIOS (Legacy boot).
*   `video_drivers_guide.sh`: Руководство по установке видео драйверов (Intel, AMD, NVIDIA) с акцентом на гибернацию и Wayland.
...
*   При установке драйверов **всегда** следуйте инструкции из файла `video_drivers_guide.sh`.
*   Убедитесь, что вы **внутри chroot** (`arch-chroot /mnt /bin/bash`) перед выполнением команд из руководства.
*   Команды `grub-mkconfig` и `mkinitcpio -P` в конце руководства `video_drivers_guide.sh` **заменяют** собой аналогичные команды из БЛОКА 10 основного макета, **если** вы пропустили их выполнение в БЛОКЕ 10.
*   **Не запускайте** `video_drivers_guide.sh` как скрипт. Копируйте и вставляйте команды вручную.
