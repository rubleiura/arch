...
#   4. Разметка диска (GPT + UEFI) + LUKS + LVM
#   5. Форматирование, LUKS, LVM, Btrfs, монтирование
#   6. Установка базовых пакетов
#   7. Настройки внутри системы (chroot)
#   8. Hostname и пароль root (chroot)
#   9. Пользователь и sudo (chroot)
#   10. Установка ядра, GRUB, mkinitcpio (chroot) ← ИСПРАВЛЕН!
#   11. Системные утилиты и настройки (chroot)
#   12. Установка видеодрайвера (chroot) (улучшенный)
...
# 2. Зашифрованный раздел для LVM (остальное место)
...
echo ""
# 💡 ОБЯЗАТЕЛЬНО:
#   После разметки проверьте, что разделы "sda1", "sda2" (или другие)
#   Соответствуют имени своего диска "sdx".
...
# ################################################################
#
# ℹ️ Зачем: Форматирование ESP, создание LUKS-контейнера, настройка LVM, создание подтомов Btrfs, монтирование.
...
# 2. Инициализируем LUKS на втором разделе
...
lvcreate -L 4G vg_main -n lv_swap # 4G swap
lvcreate -l 100%FREE vg_main -n lv_root # Остальное место для root
...
# 12. Создаем директории для монтирования других подтомов и ESP
...
pacstrap /mnt lvm2 # <-- ВАЖНО: Установить пакет lvm2
# Установка microcode и других пакетов без изменений
...
# --- ИСПРАВЛЕНИЕ: subvol=/@ → subvol=@ ---
sed -i 's|subvol=/@|subvol=@|g' /mnt/etc/fstab
clear
echo ""
echo "#####################################################"
...
# --- НАСТРОЙКА MKINITCPIO ДЛЯ LUKS + LVM ---
...
GRUB_CMDLINE_LINUX_DEFAULT="loglevel=3 quiet splash rd.shell=0 rd.emergency=halt"
GRUB_CMDLINE_LINUX="cryptdevice=UUID=${CRYPT_UUID}:cryptlvm root=/dev/vg_main/lv_root rootflags=subvol=@ resume=/dev/vg_main/lv_swap"
GRUB_PRELOAD_MODULES="part_gpt part_msdos"
GRUB_ENABLE_CRYPTODISK=y
GRUB_TIMEOUT_STYLE=menu
GRUB_DISABLE_RECOVERY=true
EOF
# --- НАСТРОЙКА GRUB-BTRFS ---
...
mkinitcpio -P
# --- ВКЛЮЧЕНИЕ СЛУЖБ ---
systemctl enable NetworkManager.service grub-btrfsd.service sshd.service plymouth-quit.service
clear
echo ""
echo "#####################################################"
...
# pacman -S --noconfirm nvidia nvidia-utils lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia vulkan-icd-loader lib32-vulkan-icd-loader
# B. Для LTS-ядра (linux-lts):
# pacman -S --noconfirm nvidia-lts nvidia-utils lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia vulkan-icd-loader lib32-vulkan-icd-loader
# C. Универсальный DKMS-драйвер (для кастомных ядер):
# pacman -S --noconfirm nvidia-dkms nvidia-utils lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia vulkan-icd-loader lib32-vulkan-icd-loader
...
# pacman -S --noconfirm vulkan-icd-loader lib32-vulkan-icd-loader # Установите зависимости
...
# 💡 (Этот шаг уже включён в варианты выше, но повторяется для ясности и если был выбран nouveau)
...
clear
pacman -Sy
pacman -S --noconfirm cinnamon cinnamon-translations blueman lightdm lightdm-gtk-greeter
pacman -S --noconfirm ffmpegthumbnailer poppler-glib
systemctl enable lightdm.service
mkinitcpio -P
clear
