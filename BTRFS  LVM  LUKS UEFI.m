...
timedatectl set-ntp true
# Reflector
# 1. Скачать зеркальный список
curl -so /etc/pacman.d/mirrorlist "https://archlinux.org/mirrorlist/?country=RU&country=FI&country=EE&country=LV&country=LT&country=SE&country=NO&protocol=https"
# 2. Расомментируем зеркала
sed -i 's/^#Server/Server/' /etc/pacman.d/mirrorlist
# 3. Запускаем reflector
reflector --protocol https --age 30 --latest 20 --sort rate --connection-timeout 10 --download-timeout 10 --save /etc/pacman.d/mirrorlist
# Включить
...
# ВАЖНО: подставляется значение CRYPT_UUID из переменной
sed -i "s|^GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX=\"cryptdevice=UUID=${CRYPT_UUID}:cryptlvm root=/dev/vg_main/lv_root rootflags=subvol=@ resume=/dev/vg_main/lv_swap\"|" /etc/default/grub
# Загрузка модулей GPT/MSDOS
sed -i 's|^GRUB_PRELOAD_MODULES=.*|GRUB_PRELOAD_MODULES="part_gpt part_msdos"|' /etc/default/grub
# Включить поддержку зашифрованного диска (раскомментировать или установить)
sed -i 's|^#*GRUB_ENABLE_CRYPTODISK=.*|GRUB_ENABLE_CRYPTODISK=y|' /etc/default/grub
# Отключить recovery-меню
...
pacman -Sy --noconfirm gstreamer gst-plugins-{base,good,bad,ugly} gst-libav ffmpeg a52dec faac faad2 flac lame libdca libdv libmad libmpeg2 libtheora libvorbis wavpack x264 x265 xvidcore libdvdcss vlc vlc-plugins-all taglib
...
pacman -Sy --noconfirm gnome-terminal nautilus vlc eog evince gedit
pacman -Sy --noconfirm network-manager-applet blueman
...
sudo pacman -Syyy
sudo pacman -Sy --noconfirm snapper snap-pac btrfsmaintenance
