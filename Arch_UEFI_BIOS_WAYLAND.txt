...
#################################################
## Оптимизация SSH перед установкой Arch Linux ##
## Этот пункт предназначен только при          ##
## установке с помощью SSH соединения          ##
#################################################
...
echo ""
echo "################################################
## <<< ПОДГОТОВКА К УСТАНОВКЕ ЗАВЕРШИЛАСЬ >>> ##
################################################"
echo ""
echo ""
...
# ===================================================
# -n 1::+1M: раздел 1, начало автоматически, размер 1M
# --typecode=1:ef02: тип EF02 (BIOS Boot)
# --change-name=1:'Раздел Bios Boot Arch Linux': имя раздела
sgdisk -n 1::+1M --typecode=1:ef02 --change-name=1:'Раздел Bios Boot Arch Linux' /dev/sda
# ===================================================
# 4. СОЗДАНИЕ EFI СИСТЕМНОГО РАЗДЕЛА (ESP)
# ===================================================
# -n 2::+1024M: раздел 2, начало после предыдущего, размер 1024M
# --typecode=2:ef00: тип EF00 (EFI System)
# --change-name=2:'Раздел Efi Arch Linux': имя раздела
sgdisk -n 2::+1024M --typecode=2:ef00 --change-name=2:'Раздел Efi Arch Linux' /dev/sda
# ===================================================
# 5. СОЗДАНИЕ КОРНЕВОГО РАЗДЕЛА (/)
# ===================================================
# -n 3::-4G: раздел 3, начало после предыдущего, размер до последних 4ГБ
# --typecode=3:8300: тип 8300 (Linux Filesystem)
# --change-name=3:'Системный раздел Arch Linux': имя раздела
sgdisk -n 3::-4G --typecode=3:8300 --change-name=3:'Системный раздел Arch Linux' /dev/sda
# ===================================================
# 6. СОЗДАНИЕ SWAP-РАЗДЕЛА
# ===================================================
# -n 4::-0: раздел 4, начало после предыдущего, размер весь остаток
# --typecode=4:8200: тип 8200 (Linux Swap)
# --change-name=4:'Раздел подкачки Arch Linux': имя раздела
sgdisk -n 4::-0 --typecode=4:8200 --change-name=4:'Раздел подкачки Arch Linux' /dev/sda
clear
echo ""
echo ""
# ===================================================
# 7. ПРОВЕРКА СОЗДАННОЙ РАЗМЕТКИ
# ===================================================
...
echo ""
echo "##############################################################
## <<< ФОРМАТИРОВАНИЕ И МОНТИРОВАНИЕ РАЗДЕЛОВ ЗАВЕРШЕНО >>> ##
##############################################################"
echo ""
echo ""
...
pacstrap /mnt memtest86+-efi memtest86+
# РЕДАКТОР: текстовый редактор
pacstrap /mnt nano
# PACMAN: оптимизация зеркал, доп. утилиты и клиент curl
...
echo ""
echo "##################################################
## <<< СОВЕРШЕН ВХОД в СИСТЕМУ CHROOT       >>> ##
##################################################"
echo ""
echo ""
...
# 4. Получение UUID SWAP раздела (/dev/sda4)
...
echo ""
echo "###############################################################
## <<<  УСТАНОВКА СИСТЕМНЫХ ПРОГРАММ И WAYLAND ЗАВЕРШЕНА >>> ##
###############################################################"
echo ""
echo ""
echo "##############################################
## <<<  ВИДЕОКАРТЫ  ДАННОГО КОМПЬЮТЕРА  >>> ##
##############################################
## <<< ВЫБЕРИТЕ ДРАЙВЕРА СОГЛАСНО ТЕСТУ >>> ##
##############################################"
echo ""
echo ""
...
### ЭТОТ  ПУНКТ ВЫПОЛЯТЬ, ТОЛЬКО, КОГДА  ####
### УСТАНОВКА ВЫПОЛНЯЕТСЯ НА VIRTUALBOX  ####
#############################################
