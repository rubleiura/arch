...
#   4. Разметка диска (GPT + BIOS Boot)
...
clear
# 💡 Раздел sda1 (BIOS Boot, EF02) НЕ форматируется и НЕ монтируется!
mkswap /dev/sda3
swapon /dev/sda3
...
# ⚠️ /boot находится ВНУТРИ Btrfs (в подтоме @), отдельного раздела НЕТ
clear
echo ""
...
pacstrap /mnt memtest86+  # <-- без -efi
pacstrap /mnt nano
pacstrap /mnt reflector pacman-contrib curl
# Установка шрифтов: GUI + консольная поддержка (включая японский и китайский)
pacstrap /mnt noto-fonts noto-fonts-emoji noto-fonts-cjk ttf-dejavu ttf-liberation kdfonts terminus-font
...
# 💡 Для BIOS+GPT: устанавливаем GRUB в MBR диска, без --efi-directory
...
# ################################################################
#
# 💡 Цель: Установить драйверы для Intel, AMD или NVIDIA.
...
# ###################################################################
#
# 💡 Уберите символ '#' ТОЛЬКО перед командами вашего варианта.
# ⚠️ Не раскомментируйте несколько вариантов одновременно!
...
# Выберите ОДИН пакет драйвера в зависимости от ядра:

# A. Для ядра linux (по умолчанию):
# pacman -S --noconfirm nvidia nvidia-utils lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia

# B. Для ядра linux-lts (если вы его установили):
# pacman -S --noconfirm nvidia-lts nvidia-utils lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia

# C. Для кастомных ядер (например, linux-zen):
# pacman -S --noconfirm nvidia-dkms nvidia-utils lib32-nvidia-utils opencl-nvidia lib32-opencl-nvidia

# 📌 ГИБРИДНАЯ ГРАФИКА (Intel/AMD + NVIDIA):
# pacman -S --noconfirm switcheroo-control
# systemctl enable switcheroo-control.service

# 📌 ТОЛЬКО ДЛЯ СИСТЕМ С ОДНОЙ NVIDIA-КАРТОЙ:
# pacman -S --noconfirm libva-nvidia-driver

clear
echo ""
echo "✅ Драйверы установлены. Перейдите к ШАГУ 2."
echo ""

# #################################################################
# ## 🧰 ШАГ 2: УСТАНОВКА ДОПОЛНИТЕЛЬНЫХ БИБЛИОТЕК (для всех GPU) ##
# #################################################################
...
clear
echo ""
echo "✅ Дополнительные библиотеки установлены. Перейдите к ШАГУ 3."
echo ""

# ####################################################################
# ## 🔧 ШАГ 3: НАСТРОЙКА NVIDIA (только если выбран вариант NVIDIA) ##
# ####################################################################
#
# 💡 Эти настройки включают режим модсета, необходимый для Wayland.

# 📌 ЕСЛИ У ВАС NVIDIA — РАСКОММЕНТИРУЙТЕ СЛЕДУЮЩИЕ СТРОКИ:
...
# ⚠️ НЕ ВЫХОДИТЕ из chroot!
#
# ➡️ ПРОДОЛЖИТЕ УСТАНОВКУ:
# Выполните следующий БЛОК 14 в этом же файле:
#   [CHROOT] УСТАНОВКА ГРАФИЧЕСКОЙ СРЕДЫ (DE/WM)
