...
# 💡 Расширено: Включены рекомендованные пакеты и настройки для улучшения функциональности, безопасности и удобства.
clear
pacman -Sy
...
pacman -S --noconfirm dosfstools ntfs-3g exfatprogs gptfdisk fuse2 fuse3 fuseiso nfs-utils cifs-utils# --- Установка и настройка D-Bus ---
pacman -S --noconfirm dbus-broker
systemctl enable dbus-broker.service
# --- Установка и настройка планировщика задач и синхронизации времени ---
pacman -S --noconfirm cronie
systemctl enable cronie.service systemd-timesyncd.service
# Настройка swappiness (уже присутствует)
echo 'vm.swappiness=10' > /etc/sysctl.d/99-swappiness.conf
# --- Установка и настройка Bluetooth ---
...
# --- Установка и настройка межсетевого экрана (безопасность + поддержка SSH) ---
pacman -S --noconfirm ufw gufw
# 🛡️ Всем входящим соединениям, не разрешенным явно, будет отказано.
ufw default deny incoming
# 🟢: Все исходящие соединения разрешены.
ufw default allow outgoing
# Разрешить SSH (опционально, раскомментируйте при необходимости)
# ufw allow 22/tcp
# Разрешение трафика для всей подсети (например, доступ к SSH из внутренней сети) (опционально, раскомментируйте при необходимости)
...
# --- Установка утилит для работы с XDG и устройствами ---
pacman -S --noconfirm xdg-utils xdg-user-dirs
xdg-user-dirs-update
pacman -S --noconfirm udisks2 udiskie polkit
# --- Установка и настройка PipeWire (звуковая подсистема) ---
...
# --- Установка мультимедийных библиотек и кодеков ---
...
pacman -S --noconfirm iproute2 dnsutils s-nail networkmanager-openvpn networkmanager-l2tp
# --- Установка утилит диагностики и информации о системе ---
...
# --- Установка и настройка snapper и snap-pac (для снапшотов) ---
...
pacman -S --noconfirm fontconfig freetype2 lib32-freetype2 ttf-font-awesome
...
# mkinitcpio-openswap
pacman -S --noconfirm mkinitcpio-openswap
sed -i '/^HOOKS=/s/(microcode )/(microcode openswap )/' /etc/mkinitcpio.conf
# sbctl (для UEFI систем с Secure Boot)
# pacman -S --noconfirm sbctl # <-- Рекомендовано, но может быть специфично. Оставлено на усмотрение пользователя.
# systemctl enable sbctl.service # <-- Активация только если sbctl установлен и используется.
# ecryptfs-utils
pacman -S --noconfirm ecryptfs-utils
# mtpfs
pacman -S --noconfirm mtpfs
# acpi, acpid
pacman -S --noconfirm acpi acpid
systemctl enable acpid.service
# cpupower
pacman -S --noconfirm cpupower
# Примечание: Скрипты для автоматического переключения профилей могут быть установлены дополнительно (например, cpupower.service или через udev rules).
clear
echo ""
echo "#####################################################"
