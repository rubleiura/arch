...
# ⚠️ Важно: Не запускайте как скрипт! Выполняйте вручную.
# 🌐 Требуется: Интернет, Live-среда Arch Linux (предпочтительно последняя)
#
# 📂 Структура:
#   1. Диагностика и подготовка среды
#   2. Разметка и форматирование диска
#   3. Монтирование и установка базовой системы
#   4. Конфигурация внутри chroot
#   5. Настройка Snapper для BTRFS
#   6. Установка DE/WM и пост-установочные действия
#   7. Рекомендации после первой загрузки
#
...
# ########################################################
#
# 🎯 Зачем: Настройка базовых параметров: часы, зеркала, обновление пакетов.
# ⚠️ Важно: Выполняется в Live-среде (до chroot).
# 📦 Включает: Оптимизацию pacman, синхронизацию времени, установку диагностических утилит.
...
# ######################################################
#
# 🎯 Зачем: Проверка оборудования перед установкой.
# ⚠️ Важно: Сравните результаты с переменными в следующем блоке.
# 📋 Показывает: Тип диска (SSD/HDD), процессор, материнскую плату, объем памяти.
...
# ########################################################
#
# 🎯 Зачем: Адаптация макета под ваше конкретное оборудование.
# ⚠️ ВАЖНО: ЭТОТ БЛОК ТРЕБУЕТ РУЧНОГО РЕДАКТИРОВАНИЯ ПЕРЕД ПРОДОЛЖЕНИЕМ.
# 🔧 Используйте функцию "Найти и заменить" вашего текстового редактора для массовой замены.

##############################################################################
##                    ⚠️ ВАЖНО: Настройка Переменных ⚠️                      ##
##############################################################################
## Этот раздел ОБЯЗАТЕЛЕН для изменения переменных перед установкой.        ##
## Несоблюдение этого требования приведет к ошибкам установки.              ##
##                                                                          ##
##    ПОРЯДОК ДЕЙСТВИЙ:                                                     ##
## 1. Сравните параметры из БЛОКА 2 с таблицей переменных ниже.             ##
## 2. ИСПОЛЬЗУЙТЕ ГРУППОВОЙ ПОИСК И ЗАМЕНУ во всем файле макета:            ##
##    • Замените 'sdx' на имя вашего диска (sda, nvme0n1 и т.д.)            ##
##    • Замените 'amd-ucode' на 'intel-ucode' если у вас процессор Intel    ##
##    • Замените 'Sony' на желаемое имя компьютера                          ##
##    • Замените '4G' на необходимый размер swap                            ##
##    • Замените языковые переменные (XXXX, YYYY, ZZZZ) на нужные значения  ##
## 3. ПОСЛЕ замены ПРОВЕРЬТЕ все блоки на корректность замен.               ##
##############################################################################
...
# ################################################
# ## 📏 БЛОК 4: РАЗМЕТКА ДИСКА (GPT + EFI) #######
# ################################################
#
# 🎯 Зачем: Создание разделов UEFI, корневого и swap.
# ⚠️ ВАЖНО: Все данные на /dev/sdx будут ПОЛНОСТЬЮ УДАЛЕНЫ!
# 🔧 Используется: sgdisk для точной разметки по GPT.
...
# #######################################################
#
# 🎯 Зачем: Форматирование разделов, создание BTRFS подтомов, монтирование.
# ⚠️ Важно: Выполняется до входа в chroot.
# 📂 Подтомы: @ (root), @home, @log, @pkg, @snapshots.
# ✅ ПЕРЕД ВЫПОЛНЕНИЕМ: Убедитесь, что имена разделов корректны!
...
# ########################################################
#
# 🎯 Зачем: Установка минимальной системы и подготовка к chroot.
# ⚠️ Важно: После этого блока мы переходим в chroot для дальнейшей настройки.
# 📦 Включает: base, btrfs-progs, microcode, nano, reflector.
...
# ########################################################
# ## 🛠️ БЛОК 7: НАСТРОЙКИ ВНУТРИ СИСТЕМЫ (chroot) ########
# ########################################################
#
# 🎯 Зачем: Базовая конфигурация системы: локали, время, зеркала.
# ⚠️ Важно: Выполняется внутри chroot после базовой установки.
# 🌐 Автоматизация: Временная зона определяется по IP, зеркала обновляются еженедельно.
...
# ########################################################
#
# 🎯 Зачем: Настройка имени системы и root-доступа.
# ⚠️ Важно: Без этого система не сможет загружаться корректно.
# 🛡️ Безопасность: Создайте надежный пароль для root.
...
echo ""
echo "##################################################"
echo "## <<<  НАСТРОЙКА HOSTNAME И ROOT ЗАВЕРШЕНА >>> ##"
echo "##################################################"
echo ""
...
# #######################################################
#
# 🎯 Зачем: Создание обычного пользователя с правами sudo.
# ⚠️ Важно: Группа wheel должна иметь права sudo в /etc/sudoers.
# 🛡️ Безопасность: Используйте надежный пароль для пользователя.
...
# #######################################################
#
# 🎯 Зачем: Настройка загрузчика и initramfs для загрузки системы.
# ⚠️ Важно: GRUB должен быть установлен в EFI раздел.
# 📦 Включает: GRUB, grub-btrfs для снапшотов, plymouth для загрузки.
...
echo "##  ПРИ ЖЕЛАНИИ ВЫ МОЖЕТЕ ВЫЙТИ ИЗ УСТАНОВКИ    ##"
echo "##  ИЛИ ПРОДОЛЖИТЬ УСТАНОВКУ ГРАФИЧЕСКОЙ СРЕДЫ  ##"
echo "##################################################"
echo ""
...
# ########################################################
#
# 🎯 Зачем: Установка необходимых системных утилит и сервисов.
# 📦 Включает: PipeWire (звук), Bluetooth, принтеры, шрифты, snapper.
# ⚙️ Настройка: Оптимизация параметров ядра, включение служб.
...
echo ""
echo "####################################################################"
echo "##   СИСТЕМНЫЕ УТИЛИТЫ И SNAPPER НАСТРОЕНЫ                        ##"
echo "##   ВНИМАНИЕ: УСТАНОВКА ВИДЕОДРАЙВЕРОВ ОБЯЗАТЕЛЬНА ПЕРЕД DE/WM!  ##"
...
# ########################################################
# ## 🎮 БЛОК 12: УСТАНОВКА ВИДЕО-ДРАЙВЕРА (chroot) #######
# ########################################################
#
# ⚠️ ВАЖНО: ЭТОТ БЛОК ЗАМЕНЕН НА ОТДЕЛЬНУЮ ИНСТРУКЦИЮ.
...
# 5. ПРОДОЛЖИТЕ УСТАНОВКУ: выберите и выполните БЛОК УСТАНОВКИ DE/WM.
# 6. ПОСЛЕ УСТАНОВКИ DE/WM ВЫПОЛНИТЕ 'exit' В ЭТОМ ТЕРМИНАЛЕ CHROOT.
#    ТОЛЬКО ТОГДА продолжайте копирование команд для размонтирования.
# ----------------------------------------------------------
...
# ##############################################
# ## 🖥️ БЛОК 13: УСТАНОВКА ГРАФИЧЕСКОЙ СРЕДЫ ###
# ##############################################
#
# 🎯 Зачем: Установка предпочитаемой графической оболочки.
# ⚠️ Важно: УБЕДИТЕСЬ, ЧТО ВИДЕОДРАЙВЕРЫ УСТАНОВЛЕНЫ И НАСТРОЕНЫ!
# 📋 Выберите ОДНУ из следующих сред
...
pacman -S --noconfirm gnome-terminal nautilus eog evince gedit
pacman -S --noconfirm ffmpegthumbnailer poppler-glib
systemctl enable lightdm.service
mkinitcpio -P
clear
...
# ########################################################
# ## ✅ БЛОК 15: РЕКОМЕНДАЦИИ ПОСЛЕ ПЕРВОЙ ЗАГРУЗКИ ######
# ########################################################
#
