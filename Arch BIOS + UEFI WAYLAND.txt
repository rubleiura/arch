...
#############################################################
# Разделы диска для монтирования #  sda1 sda2 sda3 sda4     #
#############################################################
...
# #######################################################
# ## 🗂️ БЛОК 5: РАЗМЕТКА ДИСКА (GPT + EFI + BIOS) #######
# #######################################################
#
...
sgdisk -n 2::+1024M --typecode=2:ef00 --change-name=2:'Раздел Efi Arch Linux' /dev/sdx
sgdisk -n 3::-4G --typecode=3:8300 --change-name=3:'Системный раздел Arch Linux' /dev/sdx
sgdisk -n 4::-0 --typecode=4:8200 --change-name=4:'Раздел подкачки Arch Linux' /dev/sdx
clear
echo ""
gdisk -l /dev/sdx
fdisk -l /dev/sdx
...
btrfs su cr /mnt/@pkg
umount /mnt
mount -o defaults,subvol=@ /dev/sda3 /mnt
mkdir -p /mnt/{boot,home,var/log,var/cache/pacman/pkg,var/lib/machines,var/lib/portables}
mount -o defaults,subvol=@home /dev/sda3 /mnt/home
mount -o defaults,subvol=@log /dev/sda3 /mnt/var/log
mount -o defaults,subvol=@pkg /dev/sda3 /mnt/var/cache/pacman/pkg
...
clear
pacman -Syy
pacman -S --noconfirm budgie budgie-screensaver budgie-control-center dconf-editor xorg
pacman -S --noconfirm budgie-desktop-view budgie-backgrounds network-manager-applet materia-gtk-theme papirus-icon-theme
pacman -S --noconfirm nautilus gnome-terminal vlc eog evince gedit
pacman -S --noconfirm blueman engrampa ffmpegthumbnailer libgsf udiskie evince ristretto
pacman -S --noconfirm lightdm lightdm-gtk-greeter lightdm-gtk-greeter-settings
pacman -S --noconfirm audacious audacious-plugins
pacman -S --noconfirm gvfs gvfs-afc gvfs-dnssd gvfs-goa gvfs-google gvfs-gphoto2 gvfs-mtp gvfs-nfs gvfs-onedrive gvfs-smb gvfs-wsdd gnome-keyring
