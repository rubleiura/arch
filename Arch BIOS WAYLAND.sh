# ################################################################
# #### Макет блочной установки Arch Linux (BTRFS + SNAPPER) для BIOS ######
# ################################################################
#
...
##     поиска и замены в вашем текстовом редакторе (например, Ctrl+H в Vim, ##
##     или Ctrl+H в Nano через xxd), чтобы переменные в таблице и по всему  ##
...
# Создание DOS (MBR) таблицы разделов
sgdisk -o /dev/sdx
# Создание BIOS Boot раздела (1M, тип EF02) - необходим для GRUB в BIOS
sgdisk -n 1:0:+1M -t 1:ef02 -c 1:'BIOS Boot Arch' /dev/sdx
# Создание корневого раздела (оставшееся место минус swap)
sgdisk -n 2:0:-4G -t 2:8300 -c 2:'System Arch Linux' /dev/sdx
# Создание swap раздела (оставшееся место)
...
pacstrap /mnt memtest86+ # Для BIOS используем memtest86+ вместо memtest86+-efi
pacstrap /mnt nano
...
# ВАЖНО: Для BIOS используется --target=i386-pc.
...
# Получение UUID swap для параметра ядра
SWAP_UUID=$(blkid -s UUID -o value /dev/sda3)
# Включение os-prober и добавление resume в параметры ядра
...
# Создание drop-in файла для mkinitcpio-generate-shutdown-ramfs.service
mkdir -p /etc/systemd/system/mkinitcpio-generate-shutdown-ramfs.service.d
echo -e "[Service]\nProtectSystem=full" | tee /etc/systemd/system/mkinitcpio-generate-shutdown-ramfs.service.d/override.conf > /dev/null
sed -i 's/MODULES=()/MODULES=(btrfs)/' /etc/mkinitcpio.conf
# Добавление resume в HOOKS для гибернации
...
echo "##############################################"
echo ""
lspci -nn | grep -i 'vga' # Для справки, можно раскомментировать
lsmod | grep -iE 'nvidia|amdgpu|i915' # Для справки, можно раскомментировать
echo ""
...
# ########################################################
# ## 🌌 БЛОКИ 13: УСТАНОВКА ГРАФИЧЕСКОЙ СРЕДЫ ###########
# ######################################################
#
...
#        (см. video_drivers_guide_final_no_echo.sh) перед установкой DE/WM.
...
echo ""
echo "###############################################"
echo "## <<<  УСТАНОВКА TRINITY ЗАВЕРШЕНА >>> ##"
echo "###############################################"
echo ""
# Выход из chroot
exit
